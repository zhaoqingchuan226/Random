...
#define RGI_UPSCALE
    ...
    TEXTURE2D_X(_NFO_RT);
        ...
        CBUFFER_START(RadiantGIEmittersBuffer)
            float3 _EmittersBoxMin[MAX_EMITTERS];
            float3 _EmittersBoxMax[MAX_EMITTERS];
        ...
        half3 GetVirtualEmitters(float3 wpos, half3 norm) {
            ...
            for (int k=0;k<_EmittersCount;k++) {
                float3 boxMin = _EmittersBoxMin[k];
                float3 boxMax = _EmittersBoxMax[k];
                if (all(wpos >= boxMin) && all(wpos <= boxMax)) {
                    ...
                    half distAtten = rcp(1.0 + dot2(emitterPos - wpos));
